...
perm.combine.eachtime <- lapply(perm.files.time[str_detect(perm.files.time, "pairwise")], function(f) {
  ...
  mutate(y_pos = cumsum(R2), 
         sig.label = ifelse(star_pvalue == "-" & star_padjust == "-", "", paste0(star_pvalue, "/", star_padjust)),
         p.label = ifelse(star_pvalue == "-" & star_padjust == "-", "", paste0(round(p.value,3),  "/",  round(q.value,3))),)
...
ggplot(perm.combine.eachtime, aes(x = source_file, y = R2, fill = source_file)) +
  ...
  geom_text(aes(y = y_pos, label = p.label), size = 3, color = "black", vjust = 0.1, hjust = 1) +
  scale_fill_manual(values = BASE_COLORS) +
  coord_flip() + facet_wrap(~pairs, scales = "fixed", ncol = 3) + theme_minimal() +
  labs(x = "Variables", y = expression(R^2~"(p / p adjust)"), title = "PERMANOVA - timepoint-pairs")

ggsave("Combined_timepoint.pairs.pdf", width = 10, height = 6)

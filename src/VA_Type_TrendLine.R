...
df_text_anno <- plot_df %>%
  ...
  filter(p < 0.05) %>%
  mutate( 
    fmt_p = ifelse(p < 0.001, "<0.001", sprintf("%.3f", p)),
    fmt_padj = ifelse(p.adj < 0.001, "<0.001", sprintf("%.3f", p.adj)),
    
    Clean_Label = paste0(fmt_p, "/", fmt_padj),
...
p_line_trend <- ggplot() +
  ...
  facet_wrap(~ Factor, scales = "free_y", ncol = 4, axes = "all",
  ...
  theme(
    plot.margin = ggplot2::margin(t = 10, r = 45, b = 10, l = 10, unit = "pt"),
...
ggsave("Microbiome_4_Points_Clean_Trend_Lines.pdf", plot = p_line_trend, width = 10, height = 12)

write.csv(df_segments,"Microbiome_4_Points_Clean_Trend_Lines.csv",quote = F)

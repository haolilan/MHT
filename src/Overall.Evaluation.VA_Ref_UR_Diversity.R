...
group_avg <- top_data %>%
  ...
  reshape2::melt(id.vars = c("Group", "Site"), variable.name = "Taxonomy", value.name = "Abundance")
...
### supp BL ->  H_R ->  H_M ###############
# library(ggsignif)
# time_colors   <- c("BL" = "#E15759",  "H_R" = "#9B59B6", "H_M" = "#7F7F7F")
# ##
# data.signif <-read.csv("D:/WorkProjects/Demo-MHT 2026/Results/OverallEva/VA_REF_Microbe/VA_Ref_Only.TimeDiff.wilcox.csv")%>%
#   filter(comparisons%in%c("BL/H_R","BL/H_M"))%>%select(Taxonomy=Factor,group1,group2,comparisons,p,p.adj,label)%>%
#   mutate(
#     p_fmt    = ifelse(p < 0.001, sprintf("%.2e", p), sprintf("%.3f", p)),
#     padj_fmt = ifelse(p.adj < 0.001, sprintf("%.2e", p.adj), sprintf("%.3f", p.adj)),
#     label.exact    = paste0("", p_fmt, "/", padj_fmt)
#   )
# 
# target_taxa <- data.signif %>%
#   group_by(Taxonomy) %>%
#   filter(any(p < 0.05)) %>%
#   pull(Taxonomy) %>%
#   unique()
# 
# available_taxa <- intersect(target_taxa, colnames(df.merged))
# 
# plot_data <- df.merged %>%
#   filter(Site == "VA", Group %in% c("H_R", "BL", "H_M")) %>%
#   select(Group, Clinic_ID, Site, all_of(available_taxa)) %>%
#   reshape2::melt(id.vars = c("Group", "Clinic_ID", "Site"), 
...
#     .groups = "drop"
#   )
# summary_data$Group <- factor(summary_data$Group, levels = c("H_R", "BL", "H_M"))
# 
# max_y_per_taxa <- summary_data %>%
#   group_by(Taxonomy) %>%
#   summarise(max_y = max(Mean + SE, na.rm = TRUE), .groups = "drop")
# 
# data.signif <- data.signif %>%
#   filter(label != "ns/ns" & Taxonomy %in% target_taxa) %>%
#   left_join(max_y_per_taxa, by = "Taxonomy") %>%
#   group_by(Taxonomy) %>%
#   mutate(
#     base_y = ifelse(is.na(max_y), 0.2, max_y),
#     y_pos = base_y + seq_len(n()) * pmax(base_y * 0.1, 0.0001)
#   ) %>%
#   ungroup()
# 
...
#   geom_signif(
#     data = data.signif,
#     aes(xmin = group1, xmax = group2, annotations = label.exact, y_position = y_pos),
#     inherit.aes = FALSE,
#     manual = TRUE,
#     tip_length = 0.01,
#     textsize = 2,
...
#   ) +
#   
#   facet_wrap(~Taxonomy, nrow = 4, scale = "free_y",axes="all") +
...
#   )
# write.csv(summary_data, "VA_BLHRHM_barplot.csv", row.names = FALSE, quote = FALSE)
# ggsave("VA_BLHRH_barplot.pdf", width = 12, height = 10)
...
write.csv(df.merged%>%select(Site,Shannon,CST,Group),"VA_UR_alpha.csv", quote = FALSE)
...
unpaired_test_by_site <- function(data_input, target_site, g1, g2) {
  ...
  est_val   <- median(y, na.rm = TRUE) - median(x, na.rm = TRUE) # 独立样本中位数差值
...
# 使用 mutate 创建或覆盖一个格式化后的 P 值列
stat_all_df <- stat_all_df %>%
  mutate(
    Adjusted_P_formatted = ifelse(
      Adjusted_P_value < 0.001,
      formatC(Adjusted_P_value, format = "e", digits = 2),  
      formatC(Adjusted_P_value, format = "f", digits = 3)  
    )
  )
...
plot_stat_df <- stat_all_df %>%
  ...
  ungroup() %>%
  rename(Site = Sites, label = Adjusted_P_formatted)
...
ggplot(df.merged %>% filter(Site.source != "Ref"), aes(x = CST, y = Shannon, fill = Site)) +
  ...
  stat_compare_means(aes(group = Site), method = "wilcox.test", label = "p",size=3)

ggsave(paste0(prefix, ".Shannon_CST.pdf"), width = 6, height = 4)
write.csv(df.merged %>% filter(Site.source != "Ref")%>%select(CST,Shannon,Site),
          paste0(prefix, ".Shannon_CST.csv"),quote = F,row.names = F)
...
write.csv(pcoa_plot_df,paste0(prefix, ".PCoA_Timepoint.csv"),quote = F)
...
write.csv(pcoa_plot_df,paste0(prefix, ".PCoA_CST.csv"),quote = F)
...
write.csv(count_data,"Consistence_of_CSTs_4samp.csv",quote = F)
...
p_combined_facet <- ggplot(res_plot_all, aes(x = CST, y = estimate, ymin = conf.low, ymax = conf.high, color = CST)) +
  ...
  theme(
    ...
    panel.grid.minor   = element_blank(),
    panel.grid.major = element_blank(),                                 # 去除纵向网格更突显基准线
    legend.position    = "none"
  )

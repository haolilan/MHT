...
for (group_name in names(target_datasets)) {
  ...
  sil_raw_data <- sil_plot$data
  colnames(sil_raw_data) <- c("Number_of_Clusters_k", "Average_Silhouette_Width")
  sil_raw_data$Site <- group_name

  write.csv(sil_raw_data,sprintf("SourceData_%s_best_cluster_silhouette.csv", group_name))
...
final_integrated_cluster_list <-read.csv("Consolidated_Sample_CST_Cluster_List.csv")
...
for (group_name in names(group_mapping)) {
  ...
  write.csv(plot_boxplot_df,sprintf("CST_%s_Dominant_Species_Boxplot.csv", group_name),quote = F)
}

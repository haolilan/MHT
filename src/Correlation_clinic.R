...
calc_pairwise_spearman <- function(df, vars) {
  ...
  # Apply global FDR correction across all generated pairs
  # 🌟 此时进行 FDR 校正，剔除了自己和自己的 P=0 干扰，计算出的 P_adj 极其准确！
  results <- results %>%
    mutate(P_adj = p.adjust(P_value, method = "BH")) %>%
    relocate(P_adj, .after = P_value)
...
final_results <- read_excel(paste0(target_dir, "/spearman.delta.phen2phen.xlsx"))
# ==========================================
...
p_heat_lower <- ggplot(plot_data_lower_complete, aes(x = Phenotype_var, y = Test_var, fill = Estimate_log)) +
  ...
  theme(
    ...
    plot.title = element_text(face = "bold", margin = ggplot2::margin(b = 20))
  ) +
...
write.xlsx(plot_data_lower_complete, "Sfigx.Spearman.Delta_KScore.xlsx", overwrite = TRUE)
...
write.xlsx(plot_data_lower_complete, "Fig1.d.Spearman.Delta_KScore_Hormone_Metabolite.intra.xlsx", overwrite = TRUE)

...
all_stats <- read.csv("Baseline_Response_Wilcoxon.csv")
...
df_gut_plot <- all_stats %>%
  ...
  mutate(
    ...
    # 【新增】：统一处理 P 值格式 (保留3位小数，极小值用 <0.001)
    fmt_p = ifelse(P_value < 0.001, "<0.001", sprintf("%.3f", P_value)),
    fmt_padj = ifelse(Adjusted_P_value < 0.001, "<0.001", sprintf("%.3f", Adjusted_P_value)),
    
    # 【新增】：拼接最终的标签
    P_Label = paste0(fmt_p, "/", fmt_padj)
  ) %>%
  # 移除不需要的临时列
  select(-fmt_p, -fmt_padj) 

# 清理你原来的冗余代码 (去掉了原有的 Significance 的错误赋值逻辑)

write.csv(df_gut_plot %>% select(-Plot_Accuracy), "Baseline_Response_Wilcoxon_Top30withImp.csv", row.names = FALSE)

# 对特征按重要性重新排因子水平
df_gut_plot$Clean_Feature <- factor(
  df_gut_plot$Clean_Feature, 
...
# 2. 绘图部分
# ==========================================
# 提前计算当前准确率的最大值，用于确定文字在右侧停靠的 x 坐标
max_acc <- max(df_gut_plot$MeanDecreaseAccuracy, na.rm = TRUE)

p_dual_lollipop <- ggplot(df_gut_plot, aes(x = Plot_Accuracy, y = Clean_Feature, color = Sign.col)) +
  
  geom_vline(xintercept = 0, color = "darkgray", linetype = "dashed", linewidth = 0.5) +
  ...
  # 【核心新增】：在最右侧固定位置绘制 P_Label 文本
  geom_text(
    aes(x = max_acc * 1.15, label = P_Label), # 将文本停靠在最大值的 1.15 倍处
    hjust = 0,               # 左对齐，使斜杠位置看起来整齐
    color = "black",         # 统一用黑色，画面更干净
    size = 3,
    fontface = "italic",
    show.legend = FALSE      # 避免在图例里生成字母 "a"
  ) +
  ...
  scale_x_continuous(
    ...
    limits = c(-max_acc * 1.1, max_acc * 1.6)
  ) +
...
ggsave("Dual_Direction_Plot.pdf", plot = p_dual_lollipop, width = 6.5, height = 7)
write.csv(df_gut_plot%>%select(Feature,MeanDecreaseAccuracy,Mean_NR:Enrichment_Direction,P_value:Adjusted_P_value,n1,n2),"Dual_Direction_Plot.csv",quote = F)
...
run_microbe_vs_clinical_pipeline <- function(merged_df, microbe_cols, clinical_cols, 
  ...
  # 🚨 安全拦截防御：如果运气太差，所有特征在 FDR 之后连原始 P < 0.05 的都没有，则提示并退出
  if (length(valid_microbes) == 0 || length(valid_clinicals) == 0) {
    warning("没有找到任何一个原始 P < 0.05 的显著行或列，跳过热图绘制。")
  ...
  pheatmap(r_matrix_filtered, 
           ...
           fontsize_number = 12,                  # 略微放大星标，更加醒目
           color = my_color,
           cluster_rows = TRUE, 
           cluster_cols = FALSE, 
           angle_col = 45,
           border_color = "gray92",                # 使用淡淡的淡灰色边框线，极具质感
           breaks = seq(-1, 1, length.out = 101), 
           main = heatmap_title,
           filename = output_pdf, 
           width = max(6, length(valid_clinicals) * 0.8), # 根据列数动态微调 PDF 宽度，防止标签压缩
           height = max(4, length(valid_microbes) * 0.4))  # 根据行数动态微调 PDF 高度

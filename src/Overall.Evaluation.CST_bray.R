...
write.csv(df_plot_cst,sprintf("CST_Longitudinal_Proportions_%s.csv", filter_strategy), quote=F)
...
all_logistic_report <- bind_rows(logistic_results_list) %>%
  ...
  mutate(
    ...
    raw_mark = paste(round(p_value,3), round(p.adj,3), sep = "/"),
...
final_bray_all<-read.csv("Four_Sites_Bray_Distance_RawData.csv")
final_stats_all <-read.csv("Four_Sites_Bray_Distance_stat.csv") 
...
for (site_name in unique(final_bray_all$Site)) {
  ...
  if (nrow(site_stat_filtered) > 0) {
    p_single <- p_single +
      ggpubr::stat_pvalue_manual(data = site_stat_filtered, 
                                 label = "p.adj", xmin = "group1", xmax = "group2", 
                                 y.position = "y.position", tip.length = 0.015, 
                                 bracket.size = 0.45, size = 2, color = "grey20", inherit.aes = FALSE)
  }
...
for (site_name in names(prof_filtered)) {
  ...
  pure_abundance_all <- mat_raw[rownames(mat_with_meta), , drop = FALSE]  
  ...
  # ============================================================================
  # 6. Alpha(Shannon) paired Wilcoxon ##################
  # ============================================================================
...
# Merging table ############################################
# 1. combine Pairwise_PERMANOVA_Stats

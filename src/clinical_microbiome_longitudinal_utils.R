...
run_microbiome_pipeline = function(prof_list, meta_df, threshold_map, 
  ...
  write.csv(res, file.path(out_dir, paste0(prefix, ".TimeDiff.wilcox.csv")), row.names = FALSE)
    ...
    # 4. 提取显著差异物种 (显式使用 dplyr 避免函数冲突)
  sig_taxa <- res %>% 
    dplyr::filter(p.adj < 0.05) %>% 
    dplyr::select(Site, Factor) %>% 
    dplyr::distinct()
  
  # 5. 生成 heatmap 数据
  res.heatmap <- res %>% dplyr::inner_join(sig_taxa, by = c("Site", "Factor"))
  # 6. 判断是否有显著物种，如果有则输出 CSV 并画图
  if (nrow(res.heatmap) > 0) {
    
    # 【改动1】：将保存 CSV 的操作提前，确保数据一定能输出
    write.csv(res.heatmap, file.path(out_dir, paste0(prefix, ".TimeDiff.wilcox.heatmap.csv")), row.names = FALSE)
    ...
    if (!is.null(p_heatmap)) {
      # 提前计算宽高，使代码更易读且不易出错
      plot_width  <- max(7, n_distinct(sig_taxa$Factor) * 0.25 + 2)
      plot_height <- if(is.null(group_col)) 4 else max(5, length(unique(res$Subgroup)) * length(comparisons) * 0.5)
      
      ggsave(file.path(out_dir, paste0(prefix, ".TimeDiff.wilcox.heatmap.pdf")), 
             plot = p_heatmap, width = plot_width, height = plot_height, limitsize = FALSE)
    }
    
  } else {
    # 【改动2】：如果没有显著物种，在控制台给与提示，避免你以为是代码 bug 导致没输出
    message(sprintf("[%s] 未发现显著物种 (p.adj < 0.05)，已跳过生成 Heatmap 及其数据源 CSV。", prefix))
  }
